...
Ecopath models are used to estimate maximum ingestion rates, vulnerabilities, combined respiration & non-assimilated waste, and non-predatory mortality (disease etc.).  

The literature is used to do the following:
i) allometry to differentiate respiration from non-assimilated waste,
ii) carbon:silica and carbon:iron ratios in each species (used to differentiate waste into different pools)
iii)  

Two types of Ecopath models are used:
a) unadjusted models from the literature (thanks to Roshni and Stacey for compiling)
Ballerini 2014 (WAP)
Dahood 2019 (WAP)
Gurney 2014 (PEI)
Maldonado 2016
McCormack 2019 (Prydz Bay)
Pinkerton 2010 (Ross Sea)
Subramanium 2020 (Kerguelen)
Suma 2014 
Supernand 2017



b) standardised models from Hill et al 2021
...
$$
A_{i,j}=\int_{seafloor}^{surface} \textrm{p}(B_i,z)\textrm{p}(B_j,z) dz
$$

For the present, I assume a uniform probability distributions of predators and prey within their depth ranges.
...
Ingestion rates of prey by predators is governed by the abundance and productivity of prey, the vulnerability of prey and the ingestion rates of predators given the available prey.  Their co-dependence makes reliable estimation more difficult.  Nevertheless, the vulnerability and functional relationships to be used in the steady-state model can be approximately estimated from Ecopath results. 

Using terminology for Ecopath inputs and outputs, the equations to be solved are based on the predator-prey consumption matrix such that the total consumption of a prey, i, across all predators, $\sum_j$, is equal to its production discounted by the ecotrophic efficiency:
...
In order to give equal weight to all pools, the following equation could be minimised when vulnerability can be estimated externally:

\begin{equation} \label{eq:IngestMinimisation}
SS(\vec{I})=\sum_i \left( 1 - \frac{\sum_{j} d_{i,j} B_j\sum_i \left[ g \left(\vec{B},\vec{v}_j,\vec{I}_j \right)_{i,j}I_{i,j}  \right]}
                          {EE_i\left( \frac{P_i}{B_i} \right) B_i}\right)^2
\end{equation}

However, diet information is rarely sufficient for estimating vulnerability of prey to predators.  

Instead, vulnerability and maximum ingestion rate can be combined in the estimation if the maximum ingestion rate is the maximum ingestion rate for a predator across all prey and the vulnerability then relates to the combined availability of prey with the expected production of prey, independently of other predators.  Thus, 

\begin{equation} \label{eq:IngestAndVulnerabilityMinimisation}
SS_j==\left[ g \left(\vec{B},\vec{v}_j,I_j \right)_{i,j}I_{j}-d_{i,j}\left( \frac{Q_j}{B_j} \right) \right]^2
\end{equation}


```{r EcopathFns, echo=TRUE, eval=FALSE}
...
```{r estLv, echo=TRUE, eval=FALSE}

#     minimisation ####
# and saving of ingestion rates and vulnerability matrix given the ecopath model

res<-do.call(cbind,lapply(seq(1,Npool,1),function(j,iB,iQB,Diet,units,iUseLog){
      mask<-Diet[,j]>0
      B  <- iB[mask]
      QB <- iQB[j]/units
      d  <- Diet[mask,j]
      if(sum(mask)==0) return(c(QB,Diet[,j]))
      boundLo <- c(1E-10,rep(1E-10,length(d)))
      boundHi <- c(Inf,rep(1,length(d)))
      res<-minV(searchV(B,QB,d,iUseLog,boundLo,boundHi))
      v<-Diet[,j]*0
      v[mask]<-res$v
      return(c(res$I,v))
     },In_B,In_QB,Diet,units,iUseLog))
estI<-res[1,]
estV<-res[-1,]

save(estI,estV,file="IO in/estI_V_ordinaryLS.Rdata")

# check diet matrix ####

chk<-fnCheckEstI_V(estI,estV,Diet,In_B,In_QB/units)
pchk<-unlist(chk)
pDiet<-unlist(Diet)
mask<-pchk!=0
plot(pDiet[mask],pchk[mask])
# check that most of the wayward estimates are very low proportions in the diet with very little weight in the sums of squares.

```

#### Non-Predation mortality from Ecopath data

Non-predation mortality rate per biomass (in the time units of production rate) of a species in Ecopath is the loss of production not due to predation, the latter of which is explained by the ecotrohic efficiency.



$$
m0_j=(1-EE_j)\left(\frac{P_j}{B_j}\right) \textrm{  }m.t^{-1}
$$
```{r echo=TRUE,eval=FALSE}
m0pd<-(1-unlist(eMod$params[,"EE"]))*unlist(eMod$params[,"P.B"])/365

```

As a rate, $t^{-1}$ relative to biomass

$$
M0_j=-log\left[ 1-\left(\frac{P_j}{B_j}\right)(1-EE_j) \right] \textrm{  }t^{-1}
$$
...
#### Respiration and Non-assimilated Mass

Proportion of consumption that is either defaecated (not assimilated) or respired/excreted.


$$
RNA_j=\frac{\left( Q_j-P_j \right)}{Q_j}
$$
...
```{r EcopathAnalysis, echo=TRUE,eval=FALSE}
# code brought down from above and run across all models
...
resEcopath<-lapply(Ecopath_Mods,function(EcoMod){
  ...
  rSeq<-seq(1,Npool,1)
  res<-do.call(cbind,lapply(rSeq,
                    function(j,iB,iQB,Diet,units,iUseLog){
        print(paste0(EcoMod," Pool ",j))
                    mask<-Diet[,j]>0
                    B  <- iB[mask]
                    QB <- iQB[j]/units
                    d  <- Diet[mask,j]
                    if(sum(mask)==0 | sum(B>0)==0) return(c(QB,Diet[,j]))
                    boundLo <- c(1E-10,rep(1E-10,length(d)))
                    boundHi <- c(Inf,rep(1,length(d)))
                    ...
                    if(length(r1$I)>1){
                          ...
                          r2<-c(rI,v)  # need to check if applied to more than one prey
...
```{r Iv_summary, echo=FALSE, eval=FALSE}
# summarise ingestion and vulnerability from Ecopath estimates
...
Ecopath_Mods<-list(Ant = c("Ballerini2014","Dahood2019"
                          ,"McCormack2019","Pinkerton2010")
                  ,SubAnt = c("Gurney2014_Yr2000","Hill2021"
                              ,"Subramanium2020")
                   ) # end list
                
# not used:
#  standardised: ,"Ballerini2014_Stand","Gurney2014_Stand"
#                   ,"Hill2021_Stand","Pinkerton2010_Stand"
#  general:      ,"Surma2014","Maldonado2016_Yr1900","Maldonado2016_Yr2008"

fnIngestionSumry<-function(p,mods){
  Imax<-unlist(sapply(mods,function(f,p){  # start sapply ecopath models
           eMod        <- fnReadEcopath(rootEcopath,f,ReplaceNA=TRUE)
           mask<-eMod$names[,"SOwebName"] %in% p
           if(sum(mask)>0){
              eModPools   <- which(mask)
              eModB <- eMod$params[eModPools,"B"]
               if(sum(eModB>0)>0){
                  fEcopathEst <- paste0("EcopathEst_",f,"_day_OLS.Rdata")
                  load(paste0(rootEcopathEst,fEcopathEst))
                  resI<-sum(EcopathEst$IngestionMax[eModPools]*eModB)/sum(eModB)
                  return(resI)
                  } else {return(NA)} } else {return(NA)}
        },p)) # end sapply ecopath models
  print(Imax)
  mean<-mean(Imax,na.rm=TRUE)
  min<-min(Imax,na.rm=TRUE)
  max<-max(Imax,na.rm=TRUE)
  n<-sum(!is.na(Imax))
  return(c(n,mean,min,max
           ,quantile(Imax,c(0.1,0.25,0.5,0.75,0.9),na.rm=TRUE)))
} # end function IngestionSumry

I_ant<-sapply(SOwebParams$VarName,fnIngestionSumry,Ecopath_Mods$Ant)
I_sub<-sapply(SOwebParams$VarName,fnIngestionSumry,Ecopath_Mods$SubAnt)
...
Ecopath models are available for the Atlantic, West Pacific and Central Indian sectors.  Four in the Antarctic zone, two in the Subantarctic Zone and one in the northern zone. The other Ecopath models for the Southern Ocean (Maldanoda et al 2016; Surma et al 2014) are more general in their application and were not used in this study.  Four of the regional Ecopath models were standardised for intercomparisons by Hill et al (2021).  These were not used for the estimation of parameters but used as a cross-check of the results.

The following were estimated for each group within the models:
i) average maximum ingestion rate per day 
ii) vulnerabilities of prey to the predator
iii) proportion of consumption used for production (remainder for respiration or waste)
iv) non-predation mortality rate per day
