...
p1<-fnPlotPellaTomlinson(Krill,plotColours,HollingExamples,Ylim=c(0,80))
p1
...
p2<-fnPlotPopWithGamma(Gamma,parms=parms,ProjYears=ProjYears,Xlim=c(0,ProjYears$YearsTotal),XticksN=7,Ylim=c(0,1),YticksN = 5,LegendTopLeft=c(0.83,0.165))
...
if(estGfw){
   GammaFWsinglePred<-as.vector(sapply(Pred,function(p,parms,Btarget,projYrs){
       ...
       return(optimise(find_G_FW,c(0,1),parms=parms,fwB0=fwB0,Btarget=Btarget,projYrs=projYrs)$minimum)
...
,parms,ProjYears,Xlim=c(0,60),XticksN=7,Ylim=c(0,1.25)
...
,parms,ProjYears,ChangingK=ChangingK,startWhalesX=StartWhalesRelToB0,Xlim=c(0,60),XticksN=7,Ylim=c(0,1.25)
...
,parms,ProjYears,ChangingK=ChangingK,startWhalesX=StartWhalesRelToB0,Xlim=c(0,60),XticksN=7,Ylim=c(0,2.0)
