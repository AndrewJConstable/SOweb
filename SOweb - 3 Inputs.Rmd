...
trialV<-function(v,I,iB,iQB,id,iUseLog,boundLo,boundHi){
  resI<-NULL
  resV=NULL
  resValue=NULL
  # unadjusted
  Vstart<-c(I,rep(v,length(id)))
res<-optim_ARS(par=Vstart,fn=rootFnIhat,lower=boundLo,upper=boundHi,iter=10000,iter_adapt=10,trace=FALSE,B=iB,QB=iQB,d=id,useLog=iUseLog)
  resI<-c(resI,res$par[1])
  resV<-rbind(resV,res$par[-1])
  resValue<-c(resValue,res$ofv)

    # adjust Iup
  Vstart<-c(resI[1]*1.5,resV[1,])
res<-optim_ARS(par=Vstart,fn=rootFnIhat,lower=boundLo,upper=boundHi,iter=10000,iter_adapt=10,trace=FALSE,B=iB,QB=iQB,d=id,useLog=iUseLog)
  resI<-c(resI,res$par[1])
  resV<-rbind(resV,res$par[-1])
  resValue<-c(resValue,res$ofv)
...
res<-optim_ARS(par=Vstart,fn=rootFnIhat,lower=boundLo,upper=boundHi,iter=10000,iter_adapt=10,trace=FALSE,B=iB,QB=iQB,d=id,useLog=iUseLog)
  ...
  return(list(name=c("unadj","Iup","Idown","inverseV","lastInvert"),I=resI,v=resV,value=resValue))  
} # end function
...
searchV<-function(iB,iQB,id,iUseLog,boundLo,boundHi){
  res<-lapply(seq(0.1,0.9,0.1),trialV,iQB,iB,iQB,id,iUseLog,boundLo,boundHi)
...
```{r EcopathTestFns, echo=TRUE, eval=FALSE}
...
apply(eMod$diet,2,sum,na.rm=TRUE)
...
Ecopath_Mods<-c("Gurney2014_Stand")
...
names(resEcopath)<-Ecopath_Mods
save(resEcopath,file="EcopathEstimation_day_OLS.Rdata")
...
Ecopath_Mods<-c( "Ballerini2014","Ballerini2014_Stand"
                ,"Dahood2019","Gurney2014_Yr2000","Gurney2014_Stand"
                ,"Hill2021","Hill2021_Stand"
                ,"Maldonado2016_Yr1900","Maldonado2016_Yr2008"
                ,"McCormack2019"
                ,"Pinkerton2010","Pinkerton2010_Stand"
                ,"Subramanium2020","Surma2014")
                
I<-sapply(SOwebParams$VarName,function(p){
  print(p)
  Imax<-unlist(sapply(Ecopath_Mods,function(f,p){  # start sapply ecopath models
  ...
  n<-sum(!is.na(Imax))
  return(c(n,mean,min,max,quantile(Imax,c(0.1,0.25,0.5,0.75,0.9),na.rm=TRUE)))
}) # end sapply I
